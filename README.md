# Business Intelligence & Data Analytics Portfolio

Welcome! This repository serves as a working portfolio of analytics projects demonstrating my ability to transform raw data into actionable business insights. My work focuses on revenue analysis, user behavior, market segmentation, and performance optimization. Each project reflects real-world business questions and highlights my approach to data cleaning, modeling, and insight communication.

I am currently completing the TripleTen Business Intelligence and Data Analytics program, where I am building advanced skills in Excel, SQL, data visualization, and business analysis. Please see a list of core program competancies and projects I've completed below. 

**Data Preparation & Management**
- Data Cleaning and Transformation  
- Data Validation and Quality Assurance  
- Data Structuring and Documentation  
- Working with Large Datasets  

**Analysis & Modeling**
- Exploratory Data Analysis (EDA)  
- Revenue Modeling and Forecasting  
- Conversion Funnel Analysis  
- Cohort and Retention Analysis  
- Market Segmentation Analysis  
- Trend and Performance Analysis  

**Technical Tools**
- Advanced Excel (Pivot Tables, Lookup Functions, Calculated Fields)  
- SQL (Data Extraction, Joins, Aggregations, Subqueries)  
- Tableau (Dashboard Development and Data Visualization)
- PowerBI (Dashboard Development, Data Visualization, Power Query, Basic DAX)

**Business Intelligence & Communication**
- Business Problem Framing  
- Insight Generation and Interpretation  
- KPI Development and Tracking  
- Executive-Level Insight Communication  
- Dashboard Design for Stakeholders  


## 📊 Projects

- [Sprint 1: Manhattan Rental Market Analysis](manhattan-rental-market-analysis/README.md) <sub><i>Dec. 17, 2025</i></sub>
  
### 📌 Portfolio Status
This portfolio is actively expanding as I continue developing my Business Intelligence skill set and completing advanced analytics projects.
**Next project upload:** February 18, 2026
